...
describe('Authentication',()=>{
    ...
    test('Signup request fails if the username is empty',async()=>{
        const password ="random"
        const response = await axios.post(`${HTTP_SERVER_URL}/api/v1/singup`,{
            password
        })
...
describe('User avatar Information', () =>{
    ...
    test('Get available avatars',async ()=>{
        ...
        const responseMetadata = await axios.get(`${HTTP_SERVER_URL}/api/v1/user/metadata/bulk?ids=[${userId}]`) 
        expect(responseMetadata.data.avatars.length).toBe(1)
        expect(responseMetadata.avatars.userId).toBe(userId)
    })
})
